...
export const getSymbol = (index: number) => {
  ...
  return (
    String.fromCharCode(65 + (alphaIndex % 26)) +
    (Math.floor(alphaIndex / 26) || "")
  );
};
...
export const generateProfessionalPDF = async (
  ...
  doc.text(
    `Total Beads: ${stats.reduce((acc, s) => acc + s.count, 0)}`,
    20,
    100
  );
  ...
  doc.setFontSize(8);
  doc.text(
    "Generated by ConnextTheDots - Your Ultimate Bead Art Companion",
    pageWidth / 2,
    pageHeight - 15,
    { align: "center" }
  );
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      ...
      doc.setTextColor(148, 163, 184);
      doc.text(
        `Coordinate: (${startX + 1}, ${startY + 1}) to (${endX}, ${endY})`,
        20,
        27
      );
      ...
      doc.setFontSize(9);
      doc.text(
        "Pro Tip: Place your transparent pegboard directly over this printed page to see where each bead goes.",
        pageWidth / 2,
        pageHeight - 23,
        { align: "center" }
      );
